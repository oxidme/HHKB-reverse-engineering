...
int main(int argc, char **argv) {
    ...
    printf("Firmware: App %X%d.%d%d / Boot %X%d.%d%d (running %s)\n", appfirm[0], appfirm[1],
           appfirm[2], appfirm[3], bootfirm[0], bootfirm[1], bootfirm[2], bootfirm[3],
           running ? "Boot" : "App");
    ...
    for (int mode = 0; mode < 4; mode++) {
        for (int fn = 0; fn < 2; fn++) {
            ...
            ok[mode][fn] = (get_keymap(dev, (uint8_t)mode, (uint8_t)fn, maps[mode][fn]) == 0);
            if (ok[mode][fn])
    ...
    if (f) {
        ...
        fprintf(f, "  \"serial\": \"%s\",\n", serial);
        fprintf(f, "  \"app_firmware\": \"%X%d.%d%d\",\n", appfirm[0], appfirm[1], appfirm[2],
                appfirm[3]);
        fprintf(f, "  \"boot_firmware\": \"%X%d.%d%d\",\n", bootfirm[0], bootfirm[1], bootfirm[2],
                bootfirm[3]);
