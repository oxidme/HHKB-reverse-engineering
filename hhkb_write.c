...
static int apply_and_verify(IOHIDDeviceRef dev, uint8_t mode, uint8_t fn,
			    const uint8_t *want, const char *what)
{
	printf("  %s\n", what);
...
int main(int argc, char **argv)
{
	...
	char msg[128];
	snprintf(msg, sizeof(msg), "writing key %d: %02X -> %02X",
		 key, original[key], code);
